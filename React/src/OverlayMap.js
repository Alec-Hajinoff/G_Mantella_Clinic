...
import { overlayMap } from "./ApiService";

function OverlayMap({ onHotspotSelect }) {
  const handleClick = async (svgKey) => {
    const data = await overlayMap(svgKey);

    if (data.status === "success") {
      onHotspotSelect(data.products);
    } else {
      onHotspotSelect([]);
    }
  };
    ...
    <div className="overlay-map-container">
      ...
      >
        <path
          ...
          className="hotspot"
          onClick={() => handleClick("lobe")} // <-- UPDATED
        />
        ...
        <path
          ...
          className="hotspot"
          onClick={() => handleClick("helix")} // <-- UPDATED
        />
      </svg>
    </div>
  );
